...
def backtest(trainer, states_list, prices_list, tickers=None, steps_per_day=1):
    ...
    for i, (states, prices) in enumerate(zip(states_list, prices_list)):
        ...
        while not done:
            ...
            with torch.no_grad():
                q_values = trainer.q_net(state_t)
                action = torch.argmax(q_values, dim=-1).item()  # Chọn hành động max Q
        ...
        action_counts = {
            ACTION_NAMES[a]: {
                ...
                "percentage": round(100 * (action_history == a).sum() / total_actions, 2)
            }
    ...
    trainer.q_net.train()  # Trả về mode train
    return results
